...
class WebGLFrame {
  ...
  load() {
    ...
    return new Promise((resolve) => {
      this.loadShader(["./vs1.vert", "./fs1.frag"]).then((shaders) => {
        const gl = this.gl;
        ...
        this.uniType = ["uniform4fv", "uniform2fv", "uniform2fv", "uniform1f"];
  ...
  setup() {
    ...
    const VERTEX_SIZE = 10.0; // 頂点の既定のサイズ
    /**
    ...
    this.colorCodes = [];
    this.exchangeColor(255, 192, 203);
    gl.clearColor(
      this.colorCodes[0],
      this.colorCodes[1],
      this.colorCodes[2],
  ...
  /**
   * カラーコード変換
   * @property {Array} rgba - 0〜255までの数値
   */

  exchangeColor(r, g, b) {
    this.colorCodes[0] = r / 255;
    this.colorCodes[1] = g / 255;
    this.colorCodes[2] = b / 255;
  }
}
